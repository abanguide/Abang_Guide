...
var init_profiler = __esm({
  "libs/core/src/lib/profiler.ts"() {
    ...
    Profiler = class {
      ...
      outputPath;
      threads;
      constructor({ concurrency, log: log61 = import_npmlog29.default, outputDirectory }) {
        this.events = [];
        this.logger = log61;
...
var import_fs4, import_npmlog31, import_rimraf;
      ...
      async generate(tree) {
        ...
        return async () => {
          if (isGitInitialized(this.cwd)) {
            ...
            await childProcess24.exec("git", ["init"], {
              cwd: this.cwd,
              maxBuffer: 1024
            });
          }
          if (this.args.skipInstall === void 0) {
            ...
            await childProcess24.exec(command, args, {
              cwd: this.cwd,
...
function getCurrentBranch(opts) {
  import_npmlog48.default.silly("getCurrentBranch");
  const branch = childProcess13.execSync("git", ["rev-parse", "--abbrev-ref", "HEAD"], opts);
  import_npmlog48.default.verbose("currentBranch", branch);
  return branch;
}
...
async function maybeFormatFile(filePath) {
  ...
  if (fileInfo.ignored) {
    import_npmlog49.default.silly("version", `Skipped applying prettier to ignored file: ${filePath}`);
    return;
  }
  try {
    ...
    import_npmlog49.default.silly("version", `Successfully applied prettier to updated file: ${filePath}`);
  } catch {
    import_npmlog49.default.silly("version", `Failed to apply prettier to updated file: ${filePath}`);
  }
}
async function gitAdd(changedFiles, gitOpts, execOpts) {
  ...
  import_npmlog49.default.silly("gitAdd", files);
  return childProcess14.exec("git", ["add", "--", ...files], execOpts);
}
var import_devkit5, import_fs5, import_npmlog49, import_path21, import_slash3, childProcess14, resolvedPrettier;
...
function gitCommit(message, { amend, commitHooks, signGitCommit, signoffGitCommit }, opts) {
  import_npmlog50.default.silly("gitCommit", message);
  const args = ["commit"];
  ...
  import_npmlog50.default.verbose("git", args);
  return childProcess15.exec("git", args, opts);
}
var import_npmlog50, import_os6, childProcess15;
...
function gitPush(remote, branch, opts) {
  import_npmlog51.default.silly("gitPush", remote, branch);
  return childProcess16.exec("git", ["push", "--follow-tags", "--no-verify", "--atomic", remote, branch], opts).catch((error) => {
    if (/atomic/.test(error.stderr) || process.env.GIT_REDIRECT_STDERR === "2>&1" && /atomic/.test(error.stdout)) {
      import_npmlog51.default.warn("gitPush", error.stderr);
      import_npmlog51.default.info("gitPush", "--atomic failed, attempting non-atomic push");
...
function gitTag(tag, { forceGitTag, signGitTag }, opts, command = "git tag %s -m %s") {
  import_npmlog52.default.silly("gitTag", tag, command);
  ...
  import_npmlog52.default.verbose(cmd, interpolatedArgs);
...
function isAnythingCommitted(opts) {
  import_npmlog53.default.silly("isAnythingCommitted", "");
  const anyCommits = childProcess18.execSync("git", ["rev-list", "--count", "--all", "--max-count=1"], opts);
  import_npmlog53.default.verbose("isAnythingCommitted", anyCommits);
...
function remoteBranchExists(gitRemote, branch, opts) {
  import_npmlog55.default.silly("remoteBranchExists", "");
  const remoteBranch = `${gitRemote}/${branch}`;
  try {
...
async function updateLockfileVersion(pkg2) {
  ...
  const obj = await (0, import_load_json_file3.default)(lockfilePath).catch(() => {
    import_npmlog56.default.verbose("version", `${pkg2.name} has no lockfile. Skipping lockfile update.`);
  });
  if (!obj) {
    return;
  }
...
var import_devkit6, import_load_json_file3, import_npmlog56, import_path22;
...
function getCurrentSHA(opts) {
  import_npmlog57.default.silly("getCurrentSHA", "");
  const sha = childProcess21.execSync("git", ["rev-parse", "HEAD"], opts);
  import_npmlog57.default.verbose("getCurrentSHA", sha);
  return sha;
}
var import_npmlog57, childProcess21;
...
async function getCurrentTags(execOpts, matchingPattern) {
  import_npmlog58.default.silly("getCurrentTags", "matching %j", matchingPattern);
...
var import_npm_package_arg8, import_npmlog58, childProcess22;
...
async function getProjectsWithTaggedPackages(projectNodes, projectFileMap, execOpts) {
  import_npmlog60.default.silly("getTaggedPackages", "");
...
var import_npmlog60, childProcess23;
...
async function getProjectsWithUnpublishedPackages(projectNodes, opts) {
  import_npmlog61.default.silly("getProjectsWithUnpublishedPackages", "");
  const mapper = (node) => {
    const pkg2 = getPackage(node);
    ...
    return import_pacote.default.packument(pkg2.name, opts).then(
      ...
      () => {
        import_npmlog61.default.warn("", "Unable to determine published version, assuming %j unpublished.", pkg2.name);
        return node;
      }
    );
  };
...
var import_npmlog61, import_p_map6, import_pacote;
